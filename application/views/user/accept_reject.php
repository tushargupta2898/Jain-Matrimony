    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            ...
            <div class="row" id="main" >
                ...
                <?php
                $accepted=$this->db->get_where("send_request",array("from_user"=>$this->session->userdata("email"),"accept_reject"=>"accepted"))->num_rows();
                 $rejected=$this->db->get_where("send_request",array("from_user"=>$this->session->userdata("email"),"accept_reject"=>"rejected"))->num_rows();
                ?>
                <div class="col-sm-12 col-md-12" id="content">
                  <div class="col-md-2">
                    <button type="button" class="btn btn-info" onclick="get_interest('accepted',<?php echo $this->session->userdata('update_percent');?>);">Accepted Interest (<?php echo $accepted;?>)</button>
                  </div>
                  <div class="col-md-2">
                    <button type="button" class="btn btn-danger" onclick="get_interest('rejected',<?php echo $this->session->userdata('update_percent');?>);">Rejected Interest (<?php echo $rejected;?>)</button>
                  </div>                    
                   </div>
 ...
 $(document).ready(function(){
  ...
  function get_interest(interest,update_percent){
   if(update_percent>90){
    $.ajax({
        url:"<?php echo base_url();?>get_accept_reject",
        method:"GET",
        cache:false,
        ...
        success:function(result){
          ...
          $("#interest").html("");
           if(interest=="accepted"){
            $("#interest").html("Accepted Interest");
          }
          
          if(interest=="rejected"){
            $("#interest").html("Rejected Interest");
          }
          if(obj.length>0){
          ...
          $.each(obj,function(i,index){
           ...
           var profile='<div class="panel panel-primary">'+
                      '<div class="panel-heading"><b>'+index.user_id+'</b></div>'+
                      ...
                      '</table></div>'+
                      '<div class="col-md-2"><button class="btn btn-warning" type="button" onclick="full_detail('+update_percent+',\'' + index.user_id + '\');" data-toggle="modal" data-target="#myModal">Full Details</button></div>'+
                        '</div>'+
                      '</div>';
