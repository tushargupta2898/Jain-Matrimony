...
<html>
<head>
    <title>Carrer Status Master</title>
    ...
    <div class="alert alert-success">
      ...
      <form id="user_form" class="form-horizontal" enctype="multipart/form-data" >
             ...
             <div class="panel panel-danger">
                                <div class="panel-heading"><b>Carrer Status Master</b></div>
                                <div class="panel-body">
                 <div class="form-group">
                  <label for="prod_category" class="col-sm-4 control-label">Carrer Status</label>

                  <div class="col-sm-8">
                    <input type="text" class="form-control validate[required]" name="name" id="name" placeholder="Carrer Status">
                  </div>
                </div>
   ...
   function newsubmit(){
        ...
        var valid = $("#user_form").validationEngine('validate');
             if (valid == true) {
               var data=$("#user_form").serialize();
                 ...
                 $.ajax({
                        url: "<?php echo base_url(); ?>AdminController/insert/career_status/id",
                        type: "POST",
                        data: data,
                        cache: false,
      ...
      ////////////////////////////////
       function show_list(){
        var data="list=list";
        $.ajax({
          url:"<?php echo base_url(); ?>AdminController/career_status_list",
          data:data,
          type:"get",
          cache:false,
          ...
          $.each(obj, function(i, item) {
            j=i+1;
            tr="<tr><td>"+j+"</td><td>"+item.name+"</td><td><button type='button' class='btn btn-warning' onclick='get_record("+item.id+")'><i class='fa fa-pencil'></i></button>&nbsp;&nbsp;&nbsp;<button type='button' class='btn btn-danger' onclick='delete_record("+item.id+")'><i class='fa fa-trash'></i></button></td></tr>";
      ...
      //////////////////////////////////
       function get_record(id){
         var url = "<?php echo base_url(); ?>AdminController/career_status_record?id=" + id;
      ...
      function delete_record(id) {
                                                    ...
                                                    if (r == true) {
                                                        $.ajax({
                                                            url: "<?php echo base_url(); ?>AdminController/delete_record/career_status/id",
                                                            type: "POST",
                                                            data: {id: id},
                                                            cache: false,
