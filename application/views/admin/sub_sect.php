    ...
    <div class="alert alert-success">
      ...
      <form id="user_form" class="form-horizontal" enctype="multipart/form-data" >
             ...
             <div class="panel panel-danger">
                                <div class="panel-heading"><b>Sub Sect Master</b></div>
                                <div class="panel-body">
                 <div class="form-group">
                  <label for="prod_category" class="col-sm-4 control-label">Sect</label>

                  <div class="col-sm-8">
                     <select class="form-control validate[required]" id="sect_id" name="sect_id">
                      <option value="">Select Sub Sect</option>
                      <?php
                      $sect_query=$this->db->order_by("name","asc")->get_where("sect",array("t_status"=>0));
                      foreach($sect_query->result() as $value){
                      echo "<option value='".$value->sect_id."'>".$value->name."</option>";
                    }
                      ?>
                    </select>
                  </div>
                 ...
                 <div class="form-group">
                  <label for="prod_category" class="col-sm-4 control-label">Sub Sect</label>

                  <div class="col-sm-8">
                    <input type="text" class="form-control validate[required]" name="name" id="name" placeholder="Sub Sect">
                  </div>
                </div>
   ...
   function newsubmit(){
        ...
        var valid = $("#user_form").validationEngine('validate');
             if (valid == true) {
               var data=$("#user_form").serialize();
                 ...
                 $.ajax({
                        url: "<?php echo base_url(); ?>AdminController/insert/sub_sect/sub_sect_id",
                        type: "POST",
                        data: data,
                        cache: false,
      ...
      ////////////////////////////////
       function show_list(){
        var data="list=list";
        $.ajax({
          url:"<?php echo base_url(); ?>AdminController/sub_sect_list",
          data:data,
          type:"get",
          cache:false,
      ...
      var url="<?php echo base_url(); ?>AdminController/sub_sect_record?id="+id;
      $.get(url,function(data){
        var obj=JSON.parse(data);
        if(obj.msg=="success"){
            $("#sno").val(obj.id);
            $("#fstatus").val("edit");
            $("#sect_id").val(obj.sect_id);
            $("#name").val(obj.name);
            $("#user_form").show();
            $("#add_new").hide();
            $("#result").hide();
      ...
      function delete_record(id) {
                                                    ...
                                                    if (r == true) {
                                                        $.ajax({
                                                            url: "<?php echo base_url(); ?>AdminController/delete_record/sub_sect/sub_sect_id",
                                                            type: "POST",
                                                            data: {id: id},
                                                            cache: false,
